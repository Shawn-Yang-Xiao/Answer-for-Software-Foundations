...
Proof.
  ...
  intros n m.
  assert (H: n + 0 + 0 = n).
    { Set Printing Parentheses. rewrite add_comm. simpl. rewrite add_comm. reflexivity. } 
  rewrite -> H.
  reflexivity.
Qed.
...
Theorem mul_comm_S : forall m n : nat,
  n + n * m = n * S m.
Proof.
  intros m n.  induction n as [| n' IHn']. induction m as [| m' IHm'].
  - simpl. reflexivity.
  - simpl. reflexivity.
...
Proof.
  ...
  - simpl. rewrite <- IHn'. rewrite <- mul_comm_S. reflexivity.
Qed.
  (* FILL IN HERE *)
(** [] *)
