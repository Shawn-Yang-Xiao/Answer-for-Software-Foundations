...
Theorem rev_exercise1 : forall (l l' : list nat),
  ...
  l' = rev l.
Proof.
  intros l l' H1. rewrite H1. symmetry. apply rev_involutive.
Qed.
  (* FILL IN HERE *)
(** [] *)
...
Proof.
  ...
  - simpl. intros n. induction n as [| n' IHn'].
    + reflexivity.
    + simpl.  discriminate.
  -  Abort.
...
Proof.
  ...
  - intros m eq. destruct m as [| m'] eqn:E.
    + discriminate eq.
    + apply f_equal. simpl in eq. apply IHn' in eq. apply eq.
Qed.
  (* FILL IN HERE *)
(** [] *)
...
Proof.
  intros n. induction n as [| n' IHn'].
  - simpl. intros m. destruct m as [| m'] eqn:Em'.
    + reflexivity.
    + discriminate.
  - simpl. intros m. induction m as [| m' IHm'].
    + rewrite <- plus_n_Sm. intros eq1. simpl in eq1.  discriminate eq1.
    + rewrite <- plus_n_Sm. rewrite <- plus_n_Sm.
      intros eq2. simpl in eq2. injection eq2 as H1. apply IHn' in H1. 
...
Proof.
  ...
  - destruct l as [ | t l'] eqn:El'.
    + simpl. reflexivity.
    + simpl. discriminate.
  - intros l eq. destruct l as [| t l'] eqn:El'.
    + simpl. reflexivity.
    + simpl in eq. injection eq as eq2. apply IHn' in eq2. simpl. apply eq2.
Qed.
  (* FILL IN HERE *)
(** [] *)
...
Proof.
  intros X Y l.
  induction l as [| t l'] eqn:El'.
  - intros. injection H. intros H1 H2. rewrite <- H1. rewrite <- H2. reflexivity.
  - intros. simpl in H. destruct (split l') as [l1' l2'].
...
Proof.
  intros n.
  induction n as [| n' IHn'].
  - intros m. destruct m eqn:H1.
    + reflexivity.
    + simpl. reflexivity.
  - intros m. destruct m as [| m'] eqn:H1.
    + simpl. reflexivity.
...
Proof.
  ...
  - intros m. destruct m as [| m'] eqn:Em.
    + intros p H1 H2. discriminate H1.
    + intros p H1 H2. simpl in H1. destruct p as [| p'] eqn:Ep.
      simpl. discriminate.
      simpl. simpl in H2. 
  (* FILL IN HERE *) Admitted.
(** [] *)
